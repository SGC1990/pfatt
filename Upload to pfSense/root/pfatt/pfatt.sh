...
if [ "$EAP_MODE" = "bridge" ] ; then
 ...
 # Eventually this can be remove.
  ...
  /usr/local/bin/php -r "function_exists('pfSense_ngctl_attach') && pfSense_ngctl_attach('.', '$EAP_BRIDGE_IF');"
  ...
  WPA_PARAMS="\
    ...
    ap_scan 0,\
    add_network,\
    set_network 0 ca_cert \\\"$CA_PEM\\\",\
    set_network 0 client_cert \\\"$CLIENT_PEM\\\",\
    ...
    set_network 0 private_key \\\"$PRIVATE_PEM\\\",\
    enable_network 0\
  "
